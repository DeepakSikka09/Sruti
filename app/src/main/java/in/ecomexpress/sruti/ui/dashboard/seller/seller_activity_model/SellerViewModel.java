...
public class SellerViewModel extends BaseViewModel<ISellerNavigator> {
    ...
    public final ObservableField<String> advanceShipment = new ObservableField<>("0");
    ...
    public SellerViewModel(IDataManager dataManager, ISchedulerProvider schedulerProvider) {
...
//                        totalShipment.set(String.valueOf(picked + remaining + unpicked + advance + rto1 + rto2));
    ...
    public void updateShipment(Long awb, String status, String vehicle, long date_time, boolean isAdvance) {
        getDataManager().updateScannedAWBStatus(awb, status, vehicle, date_time, isAdvance, "",0,true);
    }
    ...
    public boolean checkIfAtLeastOneScan(ArrayList<Shipment_Detail> shipment_detail) {
        for (Shipment_Detail shipmentsDetail : shipment_detail) {
            //TODO BY SUMIT RAJ COMMENT
            //if anyone is not pending ->it means may be it is picked or failed
            if (!shipmentsDetail.getStatus().contains(Constants.PENDING)) {
                return true;
            }
        }
    ...
    public boolean checkIfAllScan(ArrayList<Shipment_Detail> shipment_detail) {
        for (Shipment_Detail shipmentsDetail : shipment_detail) {
            //TODO BY SUMIT RAJ COMMENT
           //it means if any one shipment is pending  it will  not go to next page i.e. all airbill must have some status except pending
            if (shipmentsDetail.getStatus().contains(Constants.PENDING)) {
                return false;
            }
        }
    ...
    public boolean checkIfAtLeastScanSuccessfully(ArrayList<Shipment_Detail> shipment_detail) {
        for (Shipment_Detail shipmentsDetail : shipment_detail) {
            //TODO BY SUMIT RAJ COMMENT
            //it means any of airwill must be pickedup
    ...
    public void getChildApiResponse(ArrayList<Long> manifestNoArray, ProgressDialog dialogOnDataUpdate, ArrayList<Shipment_Detail> shipment_detail, SellerActivity globalShipmentListActivity) {
        fetch_count = 0;
        try {
    ...
    public LiveData<ChildCommitResponse> checkChildStatusApi(String
                                                                     authToken, ChildCommitRequest childCommitRequest) {
    ...
    public void createCommitPacketNew
            (ArrayList<Shipment_Detail> shipmentsDetails, ArrayList<Recci> recci,
             long pickup_location_id, String manifest_type, double wayLatitude, double wayLongitude) {
        try {
