...
public class Logger {
     ...
     */
    public static void errorLogging(Context context, String logMessageTag, String logMessage, Throwable throwableException) {
        if (!Log.isLoggable(logMessageTag, Log.ERROR))
            return;
...
// The d method for debug logs should be implemented in the same way as the v method for verbose logs.
    ...
    private static void logToFile(Context context, String logMessageTag, String logMessage) {
        try {
            ...
            File logFile = new File(Environment.getExternalStorageDirectory(), "sruti-v2.txt");
            if (!logFile.exists())
